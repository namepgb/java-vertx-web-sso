...
public class AdminGoogleTOTPLoginHandler implements FormLoginHandler
{
    ...
    public AdminGoogleTOTPLoginHandler(WebServerVerticle verticle, AuthProvider authProvider) {
        this.verticle = verticle;
        this.authProvider = authProvider;
        this.usernameParam = USERNAME_PARAM;
        this.passwordParam = PASSWORD_PARAM;
        this.secretParam = SECRET_PARAM;
    ...
    public void handle(RoutingContext ctx) {
        ...
        if (req.method() != HttpMethod.POST) {
            ...
            String secret = params.get(secretParam);
            if (username == null || password == null || secret == null) {
                ...
                JsonObject credentials = new JsonObject().put("username", username).put("password", password).put("secret", secret);
                this.authProvider.authenticate(credentials, res -> {
                    if (res.succeeded()) {
                        AdminGoogleTOTPUser user = (AdminGoogleTOTPUser) res.result();
                        ctx.setUser(user);
                        if (session != null) {
